...
private:
  ...
  void EditNetDescriptionDynamicFixedPoint(caffe::NetParameter* param,
      ...
      const int bw_conv, const int bw_fc, const int bw_in, const int bw_out, const int bw_bias_);
  /**
  ...
  int GetIntegerLengthBias(const string layer_name);
  ...
  // layer outputs.
  vector<int> il_in_, il_params_, il_out_,il_bias_;
  ...
  // and layer outputs.
  int bw_in_, bw_conv_params_, bw_fc_params_, bw_out_, bw_bias_;
