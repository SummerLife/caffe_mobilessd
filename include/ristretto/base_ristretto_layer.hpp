...
class BaseRistrettoLayer{
 public:
  ...
  virtual void op_data(const Dtype* data,const int cnt,char* name);
 protected:
  ...
  // activations.
  int bw_params_, bw_layer_in_, bw_layer_out_, bw_bias_params_;
  // The fractional length of dynamic fixed point numbers.
  int fl_params_, fl_layer_in_, fl_layer_out_, fl_bias_params_;
