...
void Net<Dtype>::RangeInLayers(vector<string>* layer_name,
      vector<Dtype>* max_in, vector<Dtype>* max_out, vector<Dtype>* max_param, vector<Dtype>* max_bias) {
  ...
  for (int layer_id = 0; layer_id < layers_.size(); ++layer_id) {
    if (strcmp(layers_[layer_id]->type(), "Convolution") == 0 ||
	  ...
	  BaseConvolutionLayer<float>* d = dynamic_cast<BaseConvolutionLayer<float>*>(layers_[layer_id].get());
	  
	  if(d->return_bias_term_())
//	  if((layers_[layer_id].get())->return_bias_term_())
	  {
