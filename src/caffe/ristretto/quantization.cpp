...
void Quantization::EditNetDescriptionDynamicFixedPoint(NetParameter* param,
      ...
      const int bw_fc, const int bw_in, const int bw_out,const int bw_bias) {
  for (int i = 0; i < param->layer_size(); ++i) {
    // if this is a convolutional layer which should be quantized ...
    if (layers_2_quantize.find("Convolution") != string::npos &&
      ...
      if (net_part.find("Activations") != string::npos) {
		...
		LayerParameter* param_layer = param->mutable_layer(i);
		param_layer->mutable_quantization_param()->set_fl_bias_params(bw_bias -
		  GetIntegerLengthBias(param->layer(i).name()));
		param_layer->mutable_quantization_param()->set_bw_bias_params(bw_bias);
    }
...
int Quantization::GetIntegerLengthBias(const string layer_name)
{
  int pos = find(layer_names_.begin(), layer_names_.end(), layer_name)
      - layer_names_.begin();
  return il_bias_[pos];
